...
import '../schema/mysql_schema_utils.dart';

/// MySQL implementation of the Dart-first [MigrationContext].
class MysqlMigrationContext implements MigrationContext {
  MysqlMigrationContext(this.connection);
...
''');
  ...
  Future<void> renameTable(String from, String to) async {
    ...
    await connection.execute('RENAME TABLE `$from` TO `$to`');
  }
  ...
  Future<void> renameColumn(String table, String from, String to) async {
    ...
    final nativeType =
        await inspector.columnNativeType(table, from) ?? 'TEXT';
    await connection.execute(
      'ALTER TABLE `$table` CHANGE COLUMN `$from` `$to` $nativeType',
    );
  }
  ...
  Future<void> dropIndex(String indexName, {String? table}) async {
    ...
    if (table == null) {
      throw MigrationException(
        'MySQL dropIndex requires the table name.',
      );
    }
    _assertSafeIdent(table, 'table');
  ...
  String _columnDefinition(FieldDefinition field) {
    if (field is BoolField) {
      return MysqlSchemaUtils.mapBoolColumnDef(field);
    }
    var def = MysqlSchemaUtils.mapToColumnDef(field);
