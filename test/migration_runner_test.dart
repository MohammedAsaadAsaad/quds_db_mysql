...
List<Migration> buildTaskMigrations() {
  return [
    ...
    ClosureMigration(
      ...
      down: (ctx) async {
        await ctx.dropIndex('idx_mig_tasks_priority', table: 'mig_tasks');
...
void main() {
  ...
  setUp(() async {
    await connection.execute('DROP TABLE IF EXISTS mig_tasks');
    await connection.execute('DROP TABLE IF EXISTS mig_fail_demo');
    await connection.execute('DROP TABLE IF EXISTS mig_name_check');
    await connection.execute('DROP TABLE IF EXISTS mig_raw');
    await connection.execute(
      'DROP TABLE IF EXISTS `${SchemaMigrationRunner.defaultJournalTableName}`',
    );
  });
  ...
  group('SchemaMigrationRunner (MySQL)', () {
